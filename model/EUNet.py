...
class EffUNet_b4(nn.Module):
    """ U-Net with EfficientNet-B4 encoder """
    
    def __init__(self, in_channels, num_classes):
        super(EffUNet_b4, self).__init__()

        self.efficientnet = efficientnet_b4(weights=EfficientNet_B4_Weights.DEFAULT)
        self.start_conv = nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=3, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(3),
            nn.ReLU()
        )

        self.encoder_layers = list(self.efficientnet.features.children())
        
        self.encoder1 = nn.Sequential(*self.encoder_layers[0:2])  # Output: 24x56x56
        self.encoder2 = nn.Sequential(*self.encoder_layers[2:3])  # Output: 32x28x28
        self.encoder3 = nn.Sequential(*self.encoder_layers[3:5])  # Output: 56x14x14
        self.encoder4 = nn.Sequential(*self.encoder_layers[5:9])  # Output: 160x7x7
        self.encoder5 = nn.Sequential(*self.encoder_layers[9:11]) # Output: 272x7x7
        ...
        self.head_conv = nn.Conv2d(in_channels=16, out_channels=num_classes, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        x = F.interpolate(enc5, size=enc1.shape[2:])
        x = torch.cat([x, enc4], dim=1)

        x = self.up_block_4(x)
        x = F.interpolate(x, size=enc1.shape[2:])
        ...
        x = self.up_block_1a(x)
        x = F.interpolate(x, scale_factor=2)

        x = self.up_block_1b(x)
        output = self.head_conv(x)

        return output
#model = EffUNet_b0(in_channels=3, classes=4).to(config.DEVICE)
